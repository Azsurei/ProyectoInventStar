...
import lp2.cdejava.inventstar.clientes.mysql.ResenhaMySQL;
import lp2.cdejava.inventstar.clientes.mysql.TarjetaInventstarMySQL;
import lp2.cdejava.inventstar.clientes.mysql.TransaccionMySQL;
...
public class ClientesWS {
    ...
    private ResenhaDAO daoResenha = new ResenhaMySQL();
    ...
    @WebMethod(operationName = "insertarResenha")
    public int insertarResenha(Reseña resenha) {
        int resultado = 0;
        try {
            resultado = daoResenha.insertar(resenha);
        }catch(Exception ex) {
            System.out.println(ex.getMessage());
        }
        return resultado;
