...
import lp2.cdejava.inventstar.personal.mysql.LineaOrdenVentaMySQL;
...
public class PersonalWS {
    ...
    private LineaOrdenVentaDAO daoLinea = new LineaOrdenVentaMySQL();
    private CargoDAO daoCargo= new CargoMySQL();
    
    // ORDEN VENTA
    @WebMethod(operationName = "insertarOrdenV")
    public int insertarOrdenV(OrdenVenta orden){
        int resultado = 0;
        try{
            resultado = daoOrdenVenta.insertar(orden);
        }catch(Exception ex){
    ...
    public int insertarLineaOrdenVenta(LineaOrdenVenta linea) {
        int resultado = 0;
        try {
            resultado = daoLinea.insertar(linea);
        }catch(Exception ex) {
            System.out.println(ex.getMessage());
        }
        return resultado;
    ...
    @WebMethod(operationName = "listarLineasOrdenVenta")
    public ArrayList<LineaOrdenVenta> listarLineasOrdenVenta() {
        ArrayList<LineaOrdenVenta> lineas = new ArrayList<>();
        try {
            lineas = daoLinea.listarLineaOrdenVenta();
        }catch(Exception ex) {
            System.out.println(ex.getMessage());
        }
        return lineas;
    ...
    public int eliminarLinea(LineaOrdenVenta linea) {
        int resultado = 0;
        try {
            resultado = daoLinea.eliminar(linea);
        }catch(Exception ex) {
            System.out.println(ex.getMessage());
        }
        return resultado;
    ...
    public int modificarLinea(LineaOrdenVenta linea) {
        int resultado = 0;
        try {
            resultado = daoLinea.modificar(linea);
        }catch(Exception ex) {
            System.out.println(ex.getMessage());
        }
        return resultado;
